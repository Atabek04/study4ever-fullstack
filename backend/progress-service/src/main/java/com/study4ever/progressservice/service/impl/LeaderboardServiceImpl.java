...
public class LeaderboardServiceImpl implements LeaderboardService {
    ...
    public void calculateDailyLeaderboard(LocalDate date) {
        ...
        List<LeaderboardRankings> rankings = userStats.entrySet().stream()
            ...
            .map(entry -> {
                int rank = userStats.entrySet().stream()
                    .mapToInt(e -> e.getValue().totalMinutes > entry.getValue().totalMinutes ? 1 : 0)
                    .sum() + 1;
    ...
    private static class UserDayStats {
        ...
        public UserDayStats(long totalMinutes, int sessionCount) {
